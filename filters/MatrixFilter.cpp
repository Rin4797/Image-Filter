...
Image MatrixFilter::Apply(Image& image) {
    ...
    for (int32_t x = 0; x < width; ++x) {
        for (int32_t y = 0; y < height; ++y) {
            ...
            new_red = std::min(255, std::max(matrix_[0][0] * static_cast<int>(image.GetColorValue(x - 1, y - 1).red) +
                                       ...
                                       0));
            new_green = std::min(255, std::max(matrix_[0][0] * static_cast<int>(image.GetColorValue(x - 1, y - 1).green) +
                                       ...
                                       0));
            new_blue = std::min(255, std::max(matrix_[0][0] * static_cast<int>(image.GetColorValue(x - 1, y - 1).blue) +
                                       ...
                                       0));
            filtered_image.SetColor(x, y, Color(static_cast<uint8_t>(new_red), static_cast<uint8_t>(new_blue), static_cast<uint8_t>(new_green)));
        }
    }
